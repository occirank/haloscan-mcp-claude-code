...
async function makeHaloscanRequest(endpoint, params, method) {
    ...
    try {
        ...
        console.error("Haloscan API error:", error?.response?.status, error?.response?.data);
        throw error;
    }
}
/* ------------------------------------------------------------------ */
...
export function configureHaloscanServer(server) {
    ...
    /* -------------------- KEYWORDS SIMILAR -------------------- */
    server.tool("get_keywords_similar", "Obtenir les mots-clés similaires.", getKeywordsSimilar.shape, async (params) => {
        try {
    ...
    server.tool("get_domains_overview", "Obtenir un aperçu des domaines.", getDomainsOverview.shape, async (params) => {
        try {
            const data = await makeHaloscanRequest("/domains/overview", params, "POST");
            return {
                content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
            };
        }
    ...
    /* -------------------- DOMAINS POSITIONS -------------------- */
    server.tool("get_domains_positions", "Obtenir les positions des domaines.", getDomainsPositions.shape, async (params) => {
        try {
